...
import { getRecommendations, type LoungeWithCoords, type RecommendedLounge, type VisitType, type VenueType } from "@/lib/recommendations";
...
const ForYouPage = () => {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (data) {
        setAllLounges(data.map((l: any) => ({ ...l, city_name: l.cities?.name ?? "", city_slug: l.cities?.slug ?? "" })));
      }
      setLoading(false);
    })();
  }, []);
  ...
  useEffect(() => {
    ...
    allLounges.forEach((l) => {
      ...
      else { map.set(l.city_id, { lats: [l.latitude], lngs: [l.longitude], name: l.city_name ?? "" }); }
    });
